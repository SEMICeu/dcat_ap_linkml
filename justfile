...
gen-project:
  ...
  mv {{dest}}/*.py {{pymodel}}
  uv run gen-pydantic {{gen_pydantic_args}} {{source_schema_path}} > {{pymodel}}/{{schema_name}}_pydantic.py
  @# OWL/SHACL/JSON-Schema/JSON-LD/SQL-DDL are generated by gen-project
  @# itself using the includes list in config.yaml — no separate
  @# invocations needed. Java / TypeScript / Excel / GraphQL / proto /
  @# ShEx are intentionally omitted (see config.yaml).
